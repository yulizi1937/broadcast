			...
			mAdapter.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(View view, int position) {
					...
					case R.id.itemRecyclerAwardRootLayout:
						Intent mIntentEvent = new Intent(AwardRecyclerActivity.this,AwardProfileActivity.class);
						mIntentEvent.putExtra(AppConstants.INTENTCONSTANTS.CATEGORY,AppConstants.INTENTCONSTANTS.AWARD);
						mIntentEvent.putExtra(AppConstants.INTENTCONSTANTS.ID, mArrayListAward.get(position).getmId());
...
//					position=-1;
	...
	private void showAwardCongratulateDialog(){
		MaterialDialog mMaterialDialog = new MaterialDialog.Builder(AwardRecyclerActivity.this)
        .title(getResources().getString(R.string.dialog_birthday_award_title) + " ?")
	...
	private void checkDataInAdapter() {
		Cursor mCursor = getContentResolver().query(
				DBConstant.Award_Columns.CONTENT_URI, null, null, null,
				DBConstant.Award_Columns.COLUMN_AWARD_RECEIVED_DATE_FORMATTED + " DESC");
	...
	private void saveViewPosition(int position){
		ApplicationLoader.getPreferences().setViewIdAward(String.valueOf(position));
	}
	
	private void checkReadFromDBAndUpdateToObj(){
		int position = Integer.parseInt(ApplicationLoader.getPreferences().getViewIdAward());
		if (position >= 0 && position < mArrayListAward.size()) {
			if(mArrayListAward!=null && mArrayListAward.size() > 0){
				Cursor mCursor = getContentResolver().query(DBConstant.Award_Columns.CONTENT_URI, new String[]{DBConstant.Award_Columns.COLUMN_AWARD_ID, DBConstant.Award_Columns.COLUMN_AWARD_IS_READ, DBConstant.Award_Columns.COLUMN_AWARD_IS_LIKE, DBConstant.Award_Columns.COLUMN_AWARD_LIKE_NO}, DBConstant.Award_Columns.COLUMN_AWARD_ID + "=?", new String[]{mArrayListAward.get(position).getmId()}, null);
				
				if(mCursor!=null && mCursor.getCount() >0){
					mCursor.moveToFirst();
					boolean isToNotify = false;
					if(Boolean.parseBoolean(mCursor.getString(mCursor.getColumnIndex(DBConstant.Award_Columns.COLUMN_AWARD_IS_READ)))){
						mArrayListAward.get(position).setRead(true);
						isToNotify = true;
					}
					if(Boolean.parseBoolean(mCursor.getString(mCursor.getColumnIndex(DBConstant.Award_Columns.COLUMN_AWARD_IS_LIKE)))){
						mArrayListAward.get(position).setLike(true);
						mArrayListAward.get(position).setmLikeCount(mCursor.getString(mCursor.getColumnIndex(DBConstant.Award_Columns.COLUMN_AWARD_LIKE_NO)));
						isToNotify = true;
					}
					if(isToNotify){
	...
	public void passDataToFragment(int mId, String mCategory) {
		try{
			if(mCategory.equalsIgnoreCase(AppConstants.INTENTCONSTANTS.AWARD)){
				...
				if(mId == -786){
					mCursor = mContext.getContentResolver().query(DBConstant.Award_Columns.CONTENT_URI, null, DBConstant.Award_Columns.COLUMN_AWARD_ID + "<?", new String[]{mArrayListAward.get(mArrayListAward.size()-1).getmId()}, DBConstant.Award_Columns.COLUMN_AWARD_ID + " DESC");
					isToAddOldData = true;
				}else{
					mCursor = mContext.getContentResolver().query(DBConstant.Award_Columns.CONTENT_URI, null, null, null, DBConstant.Award_Columns.COLUMN_AWARD_ID + " DESC");
				}
				if(mCursor!=null && mCursor.getCount() > 0){
					mSwipeRefreshLayout.setEnabled(true);
					mSwipeRefreshLayout.setRefreshing(true);
					mCursor.moveToFirst();
					addAwardObjectListFromDBToBeans(mCursor, false, isToAddOldData);
...
//			mPosition =  mPosition - 1;
	...
	private BottomSheet getContextMenu(final int mPosition, int mType, final String mTitle, boolean isRead, final View mView){
		...
		mBottomSheet = new BottomSheet.Builder(AwardRecyclerActivity.this).icon(Utilities.getRoundedBitmapForContextMenu(mType)).title(mTitle).sheet(R.menu.context_menu_mobcast).build();
...
//            	 contextMenuDelete(mPosition);
	...
	private void contextMenuMarkAsRead(int mPosition){
		try{
		 ...
		 getContentResolver().update(DBConstant.Award_Columns.CONTENT_URI, values, DBConstant.Award_Columns.COLUMN_AWARD_ID + "=?", new String[]{mAwardId});	
       	 mArrayListAward.get(mPosition).setRead(true);
       	 mRecyclerView.getAdapter().notifyItemChanged(mPosition);
       	 UserReport.updateUserReportApi(mAwardId, AppConstants.INTENTCONSTANTS.AWARD, AppConstants.REPORT.READ, "");
		}catch(Exception e){
			FileLog.e(TAG, e.toString());
		}
	}
	...
	private void contextMenuMarkAsUnRead(int mPosition){
		try{
		 ...
		 getContentResolver().update(DBConstant.Award_Columns.CONTENT_URI, values, DBConstant.Award_Columns.COLUMN_AWARD_ID + "=?", new String[]{mArrayListAward.get(mPosition).getmId()});
	...
	public String apiRefreshFeedAward(boolean sortByAsc, int limit){
		try {
			JSONObject jsonObj =null;
			 if(sortByAsc){
				jsonObj = JSONRequestBuilder.getPostFetchFeedAward(sortByAsc,limit, mArrayListAward != null ? mArrayListAward.get(0).getmId() : String.valueOf("0"));
			 }else{
				 jsonObj= JSONRequestBuilder.getPostFetchFeedAward(sortByAsc,limit, mArrayListAward.get(mArrayListAward.size()-2).getmId());
			 }
			if(BuildVars.USE_OKHTTP){
				return RetroFitClient.postJSON(new OkHttpClient(), AppConstants.API.API_FETCH_FEED_AWARD, jsonObj.toString(), TAG);	
			}else{
				return RestClient.postJSON(AppConstants.API.API_FETCH_FEED_AWARD, jsonObj, TAG);	
			}
		} catch (Exception e) {
	...
	public String apiRefreshFeedAction(){
		try {
			JSONObject jsonObj = JSONRequestBuilder.getPostFetchFeedAction(AppConstants.INTENTCONSTANTS.AWARD);
			if(BuildVars.USE_OKHTTP){
	...
	public class AsyncRefreshTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			super.onPreExecute();
			if (mArrayListAward == null) {
				mProgressDialog = new MobcastProgressDialog(AwardRecyclerActivity.this);
		...
		protected Void doInBackground(Void... params) {
			// TODO Auto-generated method stub
			try{
				mResponseFromApi = isRefreshFeed ? apiRefreshFeedAward(sortByAsc,limit) : apiRefreshFeedAction();
