...
//				mContentFilePath = Utilities.fbConcealDecryptFile(TAG, new File(mContentFilePath));
	...
	private void catchErrorOnFileCorrupted(){
		try{
			AlertDialog.Builder mAlertDialog = new AlertDialog.Builder(XlsDetailActivity.this);
				...
				try{
					if(!mContentIsLike){
						...
						if(isContentLiked){
							...
							mXlsLikeTv.setCompoundDrawablesWithIntrinsicBounds(R.drawable.bitmap_item_like, 0, 0, 0);
							mXlsLikeTv.setText(mContentLikeCount);
							mXlsLikeTv.setTextColor(getResources().getColor(R.color.item_activity_color));
