...
public class PdfDetailActivity extends SwipeBackBaseActivity {
	...
	private void setIntentDataToUi(){
		try{
			mPdfTitleTv.setText(mContentTitle);
			mPdfSummaryTextTv.setText(mContentDesc);
			if(!TextUtils.isEmpty(mContentLikeCount)){
				mPdfLikeTv.setText(mContentLikeCount);	
			}
			
			if(!TextUtils.isEmpty(mContentViewCount)){
				mPdfViewTv.setText(mContentViewCount);
			}
			
			mPdfByTv.setText(Utilities.formatBy(mContentBy, mContentDate, mContentTime));
			
			if(TextUtils.isEmpty(mContentLink)){
				mPdfNewsLinkLayout.setVisibility(View.GONE);
			}
			
			if(!TextUtils.isEmpty(mContentFileName)){
				mPdfFileNameTv.setText(mContentFileName);
			}else{
				mPdfFileNameTv.setVisibility(View.GONE);
			}
			
			if(!TextUtils.isEmpty(mContentFileMeta)){
				mPdfFileInfoTv.setText(mContentFileMeta);
			}else{
				mPdfFileInfoTv.setVisibility(View.GONE);
			}
...
//				mContentFilePath = Utilities.fbConcealDecryptFile(TAG, new File(mContentFilePath));
	...
	private void setOnClickListener() {
		...
		mPdfNewsLinkLayout.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				// TODO Auto-generated method stub
				Intent mIntentWebView = new Intent(PdfDetailActivity.this, WebViewActivity.class);
		...
		mPdfLikeTv.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				// TODO Auto-generated method stub
				try{
					if(!mContentIsLike){
						...
						if(isContentLiked){
							...
							mPdfLikeTv.setCompoundDrawablesWithIntrinsicBounds(R.drawable.bitmap_item_like, 0, 0, 0);
							mPdfLikeTv.setText(mContentLikeCount);
							mPdfLikeTv.setTextColor(getResources().getColor(R.color.item_activity_color));
	...
	private void refreshFeedActionFromApi(){
		if(Utilities.isInternetConnected()){
			...
			FetchActionAsyncTask mFetchActionAsyncTask = new FetchActionAsyncTask(PdfDetailActivity.this, mId, mCategory, TAG);
...
//						mContentFilePath = Utilities.fbConcealDecryptFile(TAG, new File(mContentFilePath));
						if(!TextUtils.isEmpty(mContentFilePath)){
							...
							updateLanguageChip();
							mPdfFileNameTv.setText(Utilities.getFileName(mContentFilePath));
						}	
					}
				}
			});
		}else{
			Utilities.showCrouton(PdfDetailActivity.this, mCroutonViewGroup, getResources().getString(R.string.internet_unavailable), Style.ALERT);
		}
	}
