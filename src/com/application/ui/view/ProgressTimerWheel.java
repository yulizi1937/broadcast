 ...
 */
public class ProgressTimerWheel extends View {
    ...
    /**
     * The constructor for the ProgressTimerWheel_timerwheel
     *
     * @param context
     * @param attrs
     */
    public ProgressTimerWheel(Context context, AttributeSet attrs) {
        super(context, attrs);

        parseAttributes(context.obtainStyledAttributes(attrs,
                R.styleable.ProgressTimerWheel));
    }
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.ProgressTimerWheel_timerwheel_barWidth,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressTimerWheel_timerwheel_rimWidth,
                rimWidth);

        spinSpeed = (int) a.getDimension(R.styleable.ProgressTimerWheel_timerwheel_spinSpeed,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.ProgressTimerWheel_timerwheel_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.ProgressTimerWheel_timerwheel_barColor, barColor);

        barLength = (int) a.getDimension(R.styleable.ProgressTimerWheel_timerwheel_barLength,
                barLength);

        textSize = (int) a.getDimension(R.styleable.ProgressTimerWheel_timerwheel_textSize,
                textSize);

        textColor = (int) a.getColor(R.styleable.ProgressTimerWheel_timerwheel_textColor,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.ProgressTimerWheel_timerwheel_text)) {
            setText(a.getString(R.styleable.ProgressTimerWheel_timerwheel_text));
        }

        rimColor = (int) a.getColor(R.styleable.ProgressTimerWheel_timerwheel_rimColor,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.ProgressTimerWheel_timerwheel_circleColor,
                circleColor);

        contourColor = a.getColor(R.styleable.ProgressTimerWheel_timerwheel_contourColor, contourColor);
        contourSize = a.getDimension(R.styleable.ProgressTimerWheel_timerwheel_contourSize, contourSize);
