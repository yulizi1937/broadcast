 ...
 * @hide
 */
@TargetApi(Build.VERSION_CODES.DONUT) public abstract class DiscreteSeekBarAnimatorCompat {
    public interface AnimationFrameUpdateListener {
        public void onAnimationFrame(float currentValue);
    }

    DiscreteSeekBarAnimatorCompat() {
    ...
    public static final DiscreteSeekBarAnimatorCompat create(float start, float end, AnimationFrameUpdateListener listener) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            return new DiscreteSeekBarAnimatorCompatV11(start, end, listener);
        } else {
            return new AnimatorCompatBase(start, end, listener);
        }
    }

    private static class AnimatorCompatBase extends DiscreteSeekBarAnimatorCompat {
