...
public class SearchBox extends RelativeLayout {
	...
	public SearchBox(Context context, AttributeSet attrs, int defStyle) {
		...
		inflate(context, R.layout.play_search_container, this);
		this.searchOpen = false;
		this.isMic = true;
		// this.materialMenu = (MaterialMenuView)
		// findViewById(R.id.material_menu_button);
		this.logo = (TextView) findViewById(R.id.logo);
		this.search = (AppCompatEditText) findViewById(R.id.search);
		this.results = (ListView) findViewById(R.id.results);
		this.context = context;
		this.pb = (ProgressWheel) findViewById(R.id.pb);
		this.mic = (ImageView) findViewById(R.id.mic);
		this.drawerLogo = (ImageView) findViewById(R.id.drawer_logo);
		/*
		 * materialMenu.setOnClickListener(new OnClickListener() {
		 * 
		 * @Override public void onClick(View v) { if (searchOpen) {
		 * toggleSearch(); } else { if (menuListener != null)
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			FrameLayout searchRoot = (FrameLayout) findViewById(R.id.search_root);
	...
	public void hideCircularly(Activity activity) {
		...
		final FrameLayout layout = (FrameLayout) activity.getWindow()
				.getDecorView().findViewById(android.R.id.content);
		FrameLayout root = (FrameLayout) findViewById(R.id.search_root);
...
//			closeSearch();
	...
	private void revealFrom(float x, float y, Activity a, SearchBox s) {
		...
		FrameLayout root = (FrameLayout) s.findViewById(R.id.search_root);
		Resources r = getResources();
		float px = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 96,
				r.getDisplayMetrics());
		int cx = layout.getLeft() + layout.getRight();
